...
open('tot_num', 'w').write('tot_num={}'.format(config["training"]["client_per_round"]))
open('if_end', 'w').write('if_end=false')

job_generator = JobGenerator()

if config["training"]["optimizer"] == 'adam' or config["training"]["optimizer"] == 'Adam':
    optimizer = fluid.optimizer.Adam(learning_rate=config["training"]["learning_rate"])
elif config["training"]["optimizer"] == 'sgd':
    if config["training"]["optimizer_param"]["weight_decay"] != 0:
        optimizer = fluid.optimizer.MomentumOptimizer(learning_rate=config["training"]["learning_rate"], momentum=config["training"]["optimizer_param"]["momentum"], use_nesterov=config["training"]["optimizer_param"]["nesterov"], regularization = cn_api_fluid_regularizer_L2Decay)
    else :
        optimizer = fluid.optimizer.MomentumOptimizer(learning_rate=config["training"]["learning_rate"], momentum=config["training"]["optimizer_param"]["momentum"], use_nesterov=config["training"]["optimizer_param"]["nesterov"])#, regularization = cn_api_fluid_regularizer_L2Decay)
