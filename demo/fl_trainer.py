...
epoch_id = 0
step = 0
epoch = config["training_param"]["epochs"]
count_by_step = False
if count_by_step:
...
if config["dataset"] not in LEAF:
    with logger.preprocess_data():
        train_reader = Batch(Train(trainer_id), batch_size = config["training_param"]["batch_size"])
        test_reader = Batch(Test(), batch_size = config["training_param"]["batch_size"])
else:
    with logger.preprocess_data():
...
with logger.training():
    while not trainer.stop(epoch_id - epoch):
        ...
        if config["dataset"] in LEAF:
            train_reader = Batch(LEAFTrain(trainer_id), batch_size = config["training_param"]["batch_size"])
        ...
        with logger.training_round() as t:
            ...
            if count_by_step:
                ...
                trainer.run_with_epoch(
                    train_reader, feeder, fetch=[job._target_names[0], job._target_names[1]], num_epoch=config["training_param"]["inner_step"], logger=logger, model_size=model_size)
